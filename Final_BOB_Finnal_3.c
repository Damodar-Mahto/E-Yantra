...
/********************************************************************** Global Variables **************************************************************/
unsigned int node_count = 1; //to count the no of node detected
...
volatile unsigned char flag_S1_count = 0;   //to count no of occurances of left and right turn
...
#define Y_GYRO	0x45		//internal data register where  gyro_y data is stored
...
INT8 gyro_data_y[2];        //to store the gyro. y-axis data
...
void node_detector()
{
	...
	{
		if (line_no == 1 || line_no == 6 || rmnder == 1 || rmnder == 2)
		{
			shape_type='0';
		}
	}
...
void send_to_pc()
{
	...
	UDR0=shape_type;
	_delay_ms(10);
	UDR0=' '; //not required now,but is critical
	UDR0=' '; //not required noe,but is critical
	if(flag_Shape == 1)
	{
		_delay_ms(2000);  //2000
	}
	else if (obstacle_found == '1')
	{
		_delay_ms(2000);    //1500
	}
	else
...
int main()
{
	...
	while(1)
	{
		...
		if ((SHARP1 > 125 && SHARP1 < 160) && ( flag_left_object==0 ) && (pathL > 30 && pathL < 50))      //when left object is detected. 
		{
			flag_left_object=1;
			IR1=ADC_Conversion(4);
			if(IR1 < 130)    //146//140
		...
		SHARP5=ADC_Conversion(13); 
		if ((SHARP5 > 55 && SHARP5 < 100) && (flag_right_object==0) && (pathR > 30 && pathR < 50))       //when right object is detected  //48
		{
			flag_right_object=1;
			IR5=ADC_Conversion(8);
			if (IR5 < 145)   //152
